...
describe('Check Box Page functionality check', function() {
    ...
    const desktopFiles=['Notes', 'Commands'];
    const documentsFolders=['WorkSpace', 'Office'];
    const officeFiles=['Public', 'Private', 'Classified', 'General'];
    const workspaceFiles=['React', 'Angular', 'Veu'];
    ...
    describe('smoke: Basic functionality check', function(){
        ...
        it ('should confirm basic functionality', async function() {
            ...
            await checkBoxPage.clickChB(downloadsFiles[1]);
            const fileSelected= await checkBoxPage.getCheckBoxState(downloadsFiles[1]);
            ...
            expect(actualFileColorInMessage, 'Actual and expected color of selected file in message do not match').to.be.equal("rgba(25, 135, 84, 1)");

        });
    });
    describe('regression: Basic functionality check', function(){
        it('should select all children if parent was selected', async function() {
            ...
            const expectedMessage= `You have selected : ${normalize(nodes[0])} ${normalize(desktopFiles[0])} ${normalize(desktopFiles[1])}`;
            expect(actualMessage, 'Actual and expected confirmation text do not match').to.be.equal(expectedMessage);      
        });
        it('should set parent folder to half-checked if unselect one child', async function() {
            ...
            expect(folderState, `"${tree}" is not half-checked`).to.be.equal('half');
            const actualMessage= (await checkBoxPage.getCheckedFilesList()).replace(/\s+/g, ' ').trim();
            const expectedMessage= `You have selected : ${normalize(officeFiles[0])} ${normalize(officeFiles[1])} ${normalize(officeFiles[3])}`;
            expect(actualMessage, 'Actual and expected confirmation text do not match').to.be.equal(expectedMessage);
        });
        it('should add parent folder name to confirmation text as soon as all children were selected', async function() {
            ...
            const actualMessage= (await checkBoxPage.getCheckedFilesList()).replace(/\s+/g, ' ').trim();
            const expectedMessage= `You have selected : ${normalize(workspaceFiles[2])} ${normalize(workspaceFiles[0])} ${normalize(workspaceFiles[1])} ${normalize(documentsFolders[0])}`;
            expect(actualMessage, 'Actual and expected confirmation text do not match').to.be.equal(expectedMessage);
        });
        it('should remove parent folder name from confirmation text if unselect one child', async function() {
            await checkBoxPage.expandAndDo('click', tree, nodes[1], documentsFolders[1]);
            await checkBoxPage.expandNode(documentsFolders[1]);
            await checkBoxPage.unclickChB(officeFiles[1]);
            const actualMessage= (await checkBoxPage.getCheckedFilesList()).replace(/\s+/g, ' ').trim();
            const expectedMessage= `You have selected : ${normalize(officeFiles[0])} ${normalize(officeFiles[2])} ${normalize(officeFiles[3])}`;
            expect(actualMessage, 'Actual and expected confirmation text do not match').to.be.equal(expectedMessage);
        });
        it('should remove file names from confirmation text if unselect half-checked parent folder', async function() {
            ...
            await checkBoxPage.expandNode(nodes[2]);
            await checkBoxPage.clickChB(downloadsFiles[0]);
            let actualMessage= (await checkBoxPage.getCheckedFilesList()).replace(/\s+/g, ' ').trim();
            let expectedMessage= `You have selected : ${normalize(officeFiles[0])} ${normalize(officeFiles[1])} ${uniteAndTtrimExtention(downloadsFiles[0])}`;
            expect(actualMessage, 'Actual and expected confirmation text do not match').to.be.equal(expectedMessage);
        ...
        it('should unselect all folders and files if uncheck main tree-folder', async function() {
            ...
            const actualMessage= (await checkBoxPage.getCheckedFilesList()).replace(/\s+/g, ' ').trim();
            const expectedMessage= `You have selected : ${normalize(nodes[0])} ${normalize(desktopFiles[0])} ${normalize(desktopFiles[1])} ${normalize(workspaceFiles[2])}`;
            expect(actualMessage, 'Actual and expected confirmation text do not match').to.be.equal(expectedMessage);
        ...
        it('should restore all children when parent re-selected after manual deselection', async function() {
            await checkBoxPage.expandNode(tree);
            await checkBoxPage.clickChB(nodes[2]);
            await checkBoxPage.expandNode(nodes[2]);
            await checkBoxPage.unclickChB(downloadsFiles[1]);
            ...
            await checkBoxPage.unclickChB(downloadsFiles[0]);
            ...
            let fileState= await checkBoxPage.getCheckBoxState(downloadsFiles[0]);
            expect(fileState, `"${uniteAndTtrimExtention(downloadsFiles[0])}" is not checked`).to.be.equal('check');
            fileState= await checkBoxPage.getCheckBoxState(downloadsFiles[1]);
            expect(fileState, `"${uniteAndTtrimExtention(downloadsFiles[1])}" is not checked`).to.be.equal('check');
        });
        it('should select all files in the folder with half-check state', async function() {
